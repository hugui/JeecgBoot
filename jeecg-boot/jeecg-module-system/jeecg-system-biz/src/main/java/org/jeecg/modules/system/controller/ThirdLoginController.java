...
public class ThirdLoginController {
    ...
    @Value("${oauth2.type.WECHAT_ENTERPRISE.redirect-uri:}")
    private String wechatEnterpriseOauth2RedirectUri;
    @Value("${oauth2.type.DINGTALK.redirect-uri:}")
    private String dingtalkOauth2RedirectUri;
    ...
    public String oauth2LoginCallback(@PathVariable("source") String source, @RequestParam("state") String state, HttpServletRequest request, HttpServletResponse response,
        ...
        if (CommonConstant.WECHAT_ENTERPRISE.equalsIgnoreCase(source)) {
            ...
            String redirectUri = wechatEnterpriseOauth2RedirectUri + "?tenantId=" + tenantId;
            ;
            ...
            String redirectUri = dingtalkOauth2RedirectUri + "?tenantId=" + tenantId;
            builder.append("?redirect_uri=").append(URLEncoder.encode(redirectUri, "UTF-8"));
            // 固定值为code。
            // 授权通过后返回authCode。
            builder.append("&response_type=code");
